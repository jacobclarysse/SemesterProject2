...
def loss(original, d):
    ...
    y_true = tf.convert_to_tensor(np.tile(np.array([[0,0,0,1,0]]).transpose(), (1, N)).astype("float32"))
    loss_v = bce(y_pred, y_true)/N+lambda_1*tf.math.abs(tf.math.reduce_mean(d))+lambda_2*tf.math.reduce_variance(d)
    return loss_v
